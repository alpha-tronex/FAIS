...
const TILE_ICONS: Record<string, string> = {
  ...
  /** Financial affidavit (admin home); document with text lines */
  affidavit:
    'M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5a1.125 1.125 0 0 1-1.125-1.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H9.75A3.375 3.375 0 0 0 6.375 7.5v11.25A2.25 2.25 0 0 0 8.625 21h10.5a2.25 2.25 0 0 0 2.25-2.25V9.75a2.25 2.25 0 0 0-.659-1.591l-4.5-4.5A2.25 2.25 0 0 0 15.75 3H9.75ZM9 12h6m-6 3.75h6m-6 3.75h3',
  profile:
...
export class HomePage implements OnInit, OnDestroy {
  ...
  /** Admin home uses a fixed 3-column grid (see `.tileGridAdmin` in CSS). */
  isAdminTileLayout(): boolean {
    return this.auth.getRoleTypeIdFromToken() === 5;
  }
  ...
  private buildTiles(): HomeTile[] {
    ...
    switch (role) {
      case 5:
        // Order matches home grid: 3 cols × rows — Users, Cases, Affidavit | Documents, AI Query | Upcoming, Messages
        return [
          { label: 'Users', route: '/admin/users', icon: 'users' },
          { label: 'Cases', route: '/admin/cases', icon: 'cases' },
          { label: 'Affidavit', route: '/admin/affidavit', icon: 'affidavit' },
          { label: 'Documents', route: '/documents', icon: 'documents' },
          { label: 'AI Query', route: '/admin/query', icon: 'reports' },
          { label: 'Upcoming Events', route: '/upcoming-events', icon: 'calendar' },
